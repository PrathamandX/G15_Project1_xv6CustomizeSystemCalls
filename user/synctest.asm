...
user/_synctest:     file format elf64-littleriscv
...
main(void)
{
  ...
  if(mutex_init(mutex) < 0){
   a:	4501                	li	a0,0
   c:	3de000ef          	jal	3ea <mutex_init>
  10:	4915                	li	s2,5
  12:	04054363          	bltz	a0,58 <main+0x58>
  16:	ec26                	sd	s1,24(sp)
  18:	e44e                	sd	s3,8(sp)
  ...
  for(int i = 0; i < 5; i++){
    int pid = fork();
  1a:	310000ef          	jal	32a <fork>
  1e:	84aa                	mv	s1,a0
    if(pid == 0){
  20:	c539                	beqz	a0,6e <main+0x6e>
  for(int i = 0; i < 5; i++){
  22:	397d                	addiw	s2,s2,-1
  24:	fe091be3          	bnez	s2,1a <main+0x1a>
      exit(0);
    }
  }
  ...
  for(int i = 0; i < 5; i++)
    wait(0);
  28:	4501                	li	a0,0
  2a:	310000ef          	jal	33a <wait>
  2e:	4501                	li	a0,0
  30:	30a000ef          	jal	33a <wait>
  34:	4501                	li	a0,0
  36:	304000ef          	jal	33a <wait>
  3a:	4501                	li	a0,0
  3c:	2fe000ef          	jal	33a <wait>
  40:	4501                	li	a0,0
  42:	2f8000ef          	jal	33a <wait>

  printf("Test completed\n");
  46:	00001517          	auipc	a0,0x1
  4a:	97a50513          	addi	a0,a0,-1670 # 9c0 <malloc+0x130>
  4e:	78e000ef          	jal	7dc <printf>
  exit(0);
  52:	4501                	li	a0,0
  54:	2de000ef          	jal	332 <exit>
  58:	ec26                	sd	s1,24(sp)
  5a:	e44e                	sd	s3,8(sp)
    printf("mutex_init failed\n");
  5c:	00001517          	auipc	a0,0x1
  60:	93450513          	addi	a0,a0,-1740 # 990 <malloc+0x100>
  64:	778000ef          	jal	7dc <printf>
    exit(1);
  68:	4505                	li	a0,1
  6a:	2c8000ef          	jal	332 <exit>
        printf("child %d increment %d\n", getpid(), j);
  6e:	00001997          	auipc	s3,0x1
  72:	93a98993          	addi	s3,s3,-1734 # 9a8 <malloc+0x118>
      for(int j = 0; j < 10; j++){   // keep small first
  76:	4929                	li	s2,10
        mutex_acquire(mutex);
  78:	4501                	li	a0,0
  7a:	378000ef          	jal	3f2 <mutex_acquire>
        printf("child %d increment %d\n", getpid(), j);
  7e:	33c000ef          	jal	3ba <getpid>
  82:	85aa                	mv	a1,a0
  84:	8626                	mv	a2,s1
  86:	854e                	mv	a0,s3
  88:	754000ef          	jal	7dc <printf>
        mutex_release(mutex);
  8c:	4501                	li	a0,0
  8e:	36c000ef          	jal	3fa <mutex_release>
      for(int j = 0; j < 10; j++){   // keep small first
  92:	2485                	addiw	s1,s1,1
  94:	ff2492e3          	bne	s1,s2,78 <main+0x78>
      exit(0);
  98:	4501                	li	a0,0
  9a:	298000ef          	jal	332 <exit>
...
start(int argc, char **argv)
{
  ...
  r = main(argc, argv);
  a6:	f5bff0ef          	jal	0 <main>
  exit(r);
  aa:	288000ef          	jal	332 <exit>
...
stat(const char *n, struct stat *st)
{
 ...
 1ee:	84aa                	mv	s1,a0
  ...
  r = fstat(fd, st);
 1f0:	85ca                	mv	a1,s2
 1f2:	1a0000ef          	jal	392 <fstat>
 1f6:	892a                	mv	s2,a0
  close(fd);
 1f8:	8526                	mv	a0,s1
 1fa:	168000ef          	jal	362 <close>
  return r;
 1fe:	64a2                	ld	s1,8(sp)
}
 200:	854a                	mv	a0,s2
...
00000000000003f2 <mutex_acquire>:
...
00000000000003fa <mutex_release>:
...
printint(int fd, long long xx, int base, int sgn)
{
 ...
 490:	54c50513          	addi	a0,a0,1356 # 9d8 <digits>
 494:	883e                	mv	a6,a5
...
vprintf(int fd, const char *fmt, va_list ap)
{
 ...
 726:	2b6b8b93          	addi	s7,s7,694 # 9d8 <digits>
 ...
 788:	24c90913          	addi	s2,s2,588 # 9d0 <malloc+0x140>
        for(; *s; s++)
