...
struct shmseg {
  ...
  char *pa;      // physical page backing this key
  int refcnt;    // number of attached processes
};
...
freeproc(struct proc *p)
{
  ...
  p->trapframe = 0;
  if(p->pagetable){
  shmcleanup(p);   // keep your existing shm cleanup
...
//new waitpid
int
kwaitpid(int wanted_pid, uint64 addr)
{
  struct proc *pp;
  struct proc *p = myproc();
  int found, pid;
...
int
kthread_create(uint64 fcn, uint64 arg, uint64 stack)
{
  int i, key;
  int pid;
  uint64 va;
  ...
  // copy shared-memory mappings if parent has any
  for(key = 0; key < SHM_PAGES; key++){
    if((p->shm_mask & (1U << key)) == 0)
      continue;
